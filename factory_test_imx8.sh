...
#				run_test "[GPIO] - [CN25 DB9 GPIO ports input\output]" bin/gpio/test_gpio_input_output.sh "CN25"
				run_test "[GPIO] - [GPIO ports input\output]" bin/gpio/test_gpio_input_output2.sh 
				pause 'Press any key to continue...'
				;;
			10)
				#run_test "[GPIO] - [CN28 GPIO ports input\output]" bin/vpm/test_vpm_battery.sh ""
				run_test "[Battery] - [Battery detection]" bin/vpm/test_vpm_battery2.sh ""
				pause 'Press any key to continue...'
				;;
			11)
				run_test "[GPIO] - [GPIO key interrupt]" bin/gpio/test_gpio_interrupt.sh
				pause 'Press any key to continue...'
				;;
			12)
...
#				run_test "[RTC] - [RTC time set\get]" bin/rtc/test_rtc_set_get.sh "$NTPSERVER"
				run_test "[RTC] - [RTC time set\get]" bin/rtc/test_rtc_set_get2.sh "$RTC_DEVICE"
				pause 'Press any key to continue...'
				;;
			15)
...
#				run_test "[Audio] - [Audio input\output]" bin/audio/test_audio_input_output.sh ""
				run_test "[Audio] - [Audio input\output]" bin/audio/test_audio_input_output2.sh ""
				pause 'Press any key to continue...'
				;;
			17)
#				run_test "[LVDS] - [Play video to LVDS]" bin/fb/test_lvds_play_video.sh "/home/root/video/1.mp4"
				run_test "[LVDS] - [Play video and show colorbar to LVDS]" bin/fb/test_lvds_play_video_colorbar.sh
				pause 'Press any key to continue...'
				;;
			18)
				if [[ "$Hostname" == "imx8mprsb3720a1" ]]; then
					run_test "[DUAL DISPLAY] - [Playback video and show colorbar to HDMI, show colorbar to LVDS]" bin/fb/test_dual_display_play_video_colorbar.sh
				else
					run_test "[DUAL DISPLAY] - [Show colorbar and playback audio to HDMI, play vido to MIPI-DSI]" bin/fb/test_dual_display_play_video.sh
				fi
				pause 'Press any key to continue...'
				;;
			19)
				#run_test "[HDMI] - [Playback video to HDMI]" bin/fb/test_hdmi_play_video.sh
				if [[ "$Hostname" == "imx8qmrom7720a1" ]]; then
					run_test "[HDMI] - [Playback video and show colorbar to HDMI]" bin/fb/test_hdmi_play_video_colorbar.sh
				else
					run_test "[HDMI] - [Show colorbar and playback audio to HDMI]" bin/fb/test_hdmi_colorbar.sh
				fi
				pause 'Press any key to continue...'
				;;
			20)
#				run_test "[UART] - [HW CTS\RTS flow control Send]" bin/uart/test_uart_flow_control.sh "/dev/ttymxc2"
				run_test "[UART] - [UART loopback test]" bin/uart/test_uart_loopback.sh
				pause 'Press any key to continue...'
				;;
			21)
#				run_test "[UART] - [HW CTS\RTS flow control Receive]" bin/uart/test_uart_flow_control.sh "/dev/ttymxc2"
				run_test "[UART] - [RS-485 PORT to PORT test]" bin/uart/test_uart_rs485.sh
				pause 'Press any key to continue...'
				;;
			22)
				run_test "[WDT] - [Watchdog Firmware Version]"  bin/wdt/test_wdt_firmware.sh ""
				pause 'Press any key to continue...'
				;;
			23)
				run_test "[I2C EEPROM] - [I2C EEPROM]" bin/eeprom/test_i2c_eeprom2.sh ""
				pause 'Press any key to continue...'
				;;
			24)
				run_test "[SPI FLASH] - [SPI FLASH]" bin/spi/test_spi_flash.sh ""
				pause 'Press any key to continue...'
				;;
			25)
				if [[ "$Hostname" == *"imsse01"* ]]; then
...
#				fi
				run_test "[PCIE] - [Wifi connect]" bin/pcie/test_wifi_connect.sh 

				pause 'Press any key to continue...'
				;;
			32)
#				run_test "[Wifi] - [scan connet ping test]" bin/net/test_bt_detection.sh
				run_test "[M.2-USB] - [Bluetooth device detection test]" bin/net/test_bt_detection.sh
				pause 'Press any key to continue...'
				;;
...
$eth0_boardR_IP $HOST_OR_SLAVE"
			...
			50)
				...
				;;
			51)
				run_test "[CAN bus] - [CAN bus Read and Write]" bin/can_bus/test_can_bus2.sh ""
				pause 'Press any key to continue...'
				;;
			52)
			...
			59)
				run_test "[TPM] - [TPM SELFTEST command]" bin/tpm/test_tpm_firmware.sh
				pause 'Press any key to continue...'
				;;
			60)
#				run_test "[DSI] - [Playback video to DSI]" bin/fb/test_dsi_play_video.sh
				run_test "[MIPI-DSI] - [Show colorbar to HDMI]" bin/fb/test_dsi_colorbar.sh
				pause 'Press any key to continue...'
				;;
			61)
				run_test "[CSI] - [Camera capture]" bin/csi/test_csi_camera.sh
				pause 'Press any key to continue...'
				;;
			62)
				run_test "[Ethernet] - [Ethernet 1]" bin/net/test_eth_connect.sh "eth0"
				pause 'Press any key to continue...'
				;;
			63)
				run_test "[Ethernet] - [Ethernet 2]" bin/net/test_eth_connect.sh "eth1"
				pause 'Press any key to continue...'
				;;
			64)
                                run_test "[Ethernet] - [Ethernet 3]" bin/net/test_eth_connect.sh "eth2"
                                pause 'Press any key to continue...'
                                ;;
			65)
                                run_test "[Mobile Network] - [Mobile Network device test]" bin/mobile_network/test_mobile_network_device.sh
                                pause 'Press any key to continue...'
                                ;;
			94)
			...
			95)
				...
				;;
			96)
				run_test "[CONFIG] - [Ethernet0 MAC]" bin/spi/test_spi_mac2.sh 
				pause 'Press any key to continue...'
				;;
			97)
				if [[ "$Hostname" == "imx8qxprom5620a1" || "$Hostname" == "imx8qxprom3620a1" || "$Hostname" == "imx8mprsb3720a1" ]]; then 
					run_test "[CONFIG] - [Ethernet1 MAC]" bin/spi/test_spi_mac3.sh
				else
					run_test "[CONFIG] - [Ethernet1 MAC]" bin/net/test_r8169_mac.sh
				fi
				pause 'Press any key to continue...'
				;;
			98)
...
#				fi
				run_test "[CONFIG] - [Serial number]" bin/spi/test_spi_sn.sh 
				pause 'Press any key to continue...'
				;;
			99)
				run_test "[CONFIG] - [Manufacture date]" bin/spi/test_spi_date.sh 
				pause 'Press any key to continue...'
				;;
			100)
				...
				;;
			101)
				export	USB_DEV_NUM="2"
				run_test "[UIO4030_UIO4034_UIO4036 USB] - [USB DISK]" bin/usb/test_usb_disk.sh ""
				pause 'Press any key to continue...'
				;;
			102)
				export	USB_DEV_NUM="4"
				run_test "[UIO4032 USB] - [USB DISK]" bin/usb/test_usb_disk.sh ""
				pause 'Press any key to continue...'
				;;
			103)
				export	UARTLIST="1"
				run_test "[UIO4030 UART] - [UART loopback test]" bin/uart/test_uart_loopback.sh
				pause 'Press any key to continue...'
				;;
			104)
				export	UARTLIST="1 3"
				run_test "[UIO4032_UIO4034 UART] - [UART loopback test]" bin/uart/test_uart_loopback.sh
				pause 'Press any key to continue...'
				;;
			Q|q|E|e)
				end_test
