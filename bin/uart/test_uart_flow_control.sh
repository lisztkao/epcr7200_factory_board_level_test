...
TEST_NAME="[UART] - [HW CTS\RTS flow control Receive]"
TEST_PARAMS="$1"
...
function Test_Function
{
	...
	chmod o+rw /dev/ttymxc2
	if [ -f ttymxc2.txt ]; then
	   rm ttymxc2.txt
	fi

	stty -F $UART_PORT 1200 cs7 -parodd parenb -cstopb -icanon -iexten -ixon -ixoff crtscts cread clocal echo -echoe echok -echoctl

	sleep 10

	cat /dev/ttymxc2 > ttymxc2.txt &

	cat_pid=$!

	sync

	echo "1234567890abcdefghijklmnopqrstuvwxyz!" >/dev/ttymxc2

	sleep 10

	sync

	get_data=$(head -n 1 ttymxc2.txt) 
        sync
