...
TEST_NAME="[TPM] - [TPM SELFTEST command]"
TEST_PARAMS=""
TPM_TEST_TOOL="bin/tpm/tpm_test"
TPM_SELFTEST_RESPONSE="80010000000A00000000"

function Test_Function
{
	RETVAL=0
	$TPM_TEST_TOOL | tee /tmp/tpm_result
	if [ "`cat /tmp/tpm_result | grep $TPM_SELFTEST_RESPONSE`" == "" ];then
		RETVAL=1
	fi
	return $RETVAL
}
