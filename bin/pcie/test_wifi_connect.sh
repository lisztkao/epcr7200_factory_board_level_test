...
TEST_PARAMS=""
TEST_NAME="[PCIE] - [Wifi connect]"
read -p "Please input SSID: " WLAN_SSID
read -p "Please input Password:  " WLAN_PASSWORD




Turn_Off_Lan() {
	echo -e "\n===Close other ethernet interfaces==="
...
Connect() {
	i=30
	if [ -z "`ifconfig -a | grep $WLAN_DEV`" ]; then
		echo -e "\n===No such $WLAN_DEV==="
		return 1 
	fi
	...
	wpa_passphrase "$WLAN_SSID" "$WLAN_PASSWORD" > /tmp/wpa.conf
	wpa_supplicant -B -c /tmp/wpa.conf -i $WLAN_DEV
	while [ -z "`dmesg | tail -n 3 | grep $WLAN_DEV | grep "link becomes ready"`" ];
	do
		echo "Waiting for wpa_supplicant to $WLAN_DEV ready( $i secs)"
		i=$(($i-1))
		sleep 1
                if [ "${i}" -eq 0 ];then
...
PING() {
	echo -e "\n===Start pinging $1==="
	LOSS_RATE="0%"
	ping -I $WLAN_DEV $1 -c 5 | tee /tmp/ping_log
