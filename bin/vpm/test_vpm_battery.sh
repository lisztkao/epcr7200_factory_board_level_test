...
TEST_NAME="[VPM] - [VPM Battery test]"
TEST_PARAMS=""
TEST_PROMPT_PRE="Please plug the Battery into CN7"


function Test_Function
{
	Batt_present=`cat /sys/class/power_supply/battery/present`
	if (( $Batt_present == 1 )); then
		Batt_capacity=`cat /sys/class/power_supply/battery/capacity` 
		if (( $Batt_capacity > 0 )) && (( $Batt_capacity <= 100 )); then
			echo "Battery detection OK!"
			return 0
		else
