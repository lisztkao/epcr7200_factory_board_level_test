...
TEST_NAME="[SPI FLASH] - [SPI FLASH]"
TEST_PARAMS=""
SIZE=1024
do_test () {
	RESULT=0	
	SPIDEV="/dev/mtdblock$1"

	if [ ! -e $SPIDEV ]; then 
		echo "$SPIDEV not exist"
		return 1
	fi

	dd if=/dev/urandom of=/tmp/data bs=1 count=$SIZE &>/dev/null
	echo ""
	echo -e "$SPIDEV start testing"
	echo -ne "\t" "backing up..."
	dd if=$SPIDEV of=/tmp/dataX bs=1 count=$SIZE &>/dev/null
	echo "done"
	
	echo -ne "\t" "writing $SIZE bytes data ..."
	dd if=/tmp/data of=$SPIDEV bs=1 &>/dev/null
	echo "done"

	echo -ne "\t" "reading & comparing ..."
	dd if=$SPIDEV of=/tmp/data_r bs=1 count=$SIZE &>/dev/null
	echo "done"
	...
	echo -ne "\t" "restoring ..."
	dd if=/tmp/dataX of=$SPIDEV bs=1&>/dev/null
	echo "done"
